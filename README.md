...
1. Скрипт отслеживает изменения URL на YouTube (SPA-навигация) с дебаунсом
2. При обнаружении страницы видео (`/watch`, `/shorts`, `/embed`, `/live`) извлекает ID видео
3. Проверяет, не было ли это видео уже отправлено (in-memory кэш)
4. Формирует GET-запрос, заменяя placeholders на реальные данные
5. Отправляет запрос на вебхук с таймаутом 10 секунд
